...
func GeraRelatorioPDF(vendas []models.Venda, nomeArquivo string) error {
	...
	// Cria o tradutor para lidar com acentos (UTF-8 para ISO-8859-1)
	...
	// Usando tr() para "RELATÓRIO ANALÍTICO"
	pdf.CellFormat(190, 15, tr("RELATÓRIO ANALÍTICO DE VENDAS"), "", 1, "C", true, 0, "")
	pdf.Ln(5)
	...
	drawCard := func(x, y float64, titulo, valor string) {
		...
		pdf.SetXY(x+2, y+4)
		pdf.SetFont("Arial", "", 9)
		pdf.Cell(40, 5, tr(titulo))
		pdf.SetXY(x+2, y+10)
		pdf.SetFont("Arial", "B", 11)
		if len(valor) > 16 {
			valor = valor[:13] + "..."
		}
		pdf.Cell(40, 5, tr(valor))
	}
	...
	for _, item := range ranking {
		...
		pdf.CellFormat(120, 8, tr(item.Nome), "1", 0, "L", true, 0, "")
	...
	pdf.Ln(5)
	pdf.SetFont("Arial", "B", 12)
	pdf.Cell(0, 10, tr("Distribuição Regional:"))
	...
	pdf.Ln(8)
	pdf.MultiCell(0, 5, tr(strEstados), "", "L", false)

	// --- GRÁFICO DE BARRAS ---
	pdf.Ln(10)
	pdf.SetFont("Arial", "B", 14)
	pdf.Cell(40, 10, tr("Gráfico de Performance (Top 5)"))
	pdf.Ln(12)
	...
	for i, item := range ranking {
		...
		pdf.CellFormat(barWidth, 5, tr(item.Nome), "", 0, "C", false, 0, "")
